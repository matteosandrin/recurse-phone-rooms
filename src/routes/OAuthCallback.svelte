  ...
  onMount(async () => {
    try {
      ...
      if (codeArray.includes(code)) {
        ...
        error = 'This authorization code has already been used. Please start the login process again.';
        tokenError = true;
        loading = false;
        return;
      }
      ...
      if (!response.ok) {
        ...
        if (errorData.details && errorData.details.error === 'invalid_grant') {
          error = 'Your login session has expired. Please try logging in again.';
          tokenError = true;
        } else {
          error = errorData.error || 'Failed to authenticate with Recurse Center';
        }
        loading = false;
        return;
      }
  ...
  <div class="max-w-md w-full space-y-8 p-6 bg-white shadow-md rounded-lg">
    {#if loading}
      <div class="text-center">
        <h2 class="text-xl font-semibold mb-4">Authenticating with Recurse Center...</h2>
    ...
    {:else if error}
      <div class="text-center">
        <h2 class="text-xl font-semibold mb-2 text-red-600">Authentication Error</h2>
        <p class="text-gray-700 mb-4">{error}</p>

        {#if tokenError}
          <a href="/" class="inline-block px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
            Return to Login
          </a>
        {:else}
